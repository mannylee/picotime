...
- **The "Start at Login" toggle uses `SMAppService.mainApp`** (from `ServiceManagement`, macOS 13+ — matches the deployment target, so no legacy `SMLoginItemSetEnabled` helper bundle). The menu item's checkmark is driven off `SMAppService.mainApp.status`, applied both once at launch (so the first menu open is already correct) and in `menuWillOpen` (`AppDelegate` is the menu's delegate) so it stays in sync with System Settings. `refreshLoginItemState` treats `.enabled` and `.requiresApproval` as checked — the latter (registered but pending the user's confirmation in System Settings, common for the ad-hoc dev build) also relabels the item to prompt approval, instead of misleadingly showing no tick after the user turned it on. `SMAppService` registers *this bundle's on-disk path*, and `build.sh` recreates the bundle each build — so a durable login item means running from a stable location like `/Applications`.
